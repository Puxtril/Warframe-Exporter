...
{
	class ModelReader281 : public ModelReader
	{
		ModelReader281() = default;

	public:
		inline static ModelReader281* getInstance()
		{
			static ModelReader281* instance = new ModelReader281();
			return instance;
		}
		...
		inline std::vector<std::tuple<LotusLib::Game, int>> getEnumMapKeys() const override
		{
			std::vector<std::tuple<LotusLib::Game, int>> extTypes = {
				{ LotusLib::Game::WARFRAME, (int)ModelRiggedType::MODEL_RIGGED_281},
			};
			return extTypes;
		}
