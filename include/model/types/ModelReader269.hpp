...
namespace WarframeExporter
{
	namespace Model
	{
		class ModelReader269
		{
		public:
			static void readHeaderDebug(BinaryReaderBuffered* headerReader, const Ensmallening& ensmalleningData, const CommonFileHeader& header)
			{
			...
			static void readHeader(BinaryReaderBuffered* headerReader, const Ensmallening& ensmalleningData, const CommonFileHeader& header, ModelHeaderExternal& outHeader)
			{
			...
			static void readBodyDebug(const ModelHeaderExternal& extHeader, BinaryReaderBuffered* bodyReader)
			{
				...
				delete[] bodyReader->readFloatArray(16 * extHeader.getBoneTree().size());
