...
ModelExporterGltf::addModelDataRigged(const ModelHeaderInternal& header, const ModelBodyInternal& body)
{
	...
	gltf::Attributes vertsAttrs = addVertexDataRigged(body, header.vertexCount);
...
ModelExporterGltf::addModelDataStatic(const ModelHeaderInternal& header, const ModelBodyInternal& body)
{
	...
	gltf::Attributes vertsAttrs = addVertexDataStatic(body, header.vertexCount);
...
ModelExporterGltf::addInverseBindMatrices(const std::vector<BoneTreeNodeInternal>& boneTree, const std::vector<size_t>& weightedIndices)
{
	...
	matAcc.type = gltf::Accessor::Type::Mat4;
	matAcc.componentType = gltf::Accessor::ComponentType::Float;
...
std::vector<int32_t>
ModelExporterGltf::createMeshes(const std::vector<MeshInfoInternal>& meshInfos, gltf::Attributes attrs, int32_t indicesBuffViewIndex)
{
	gltf::Mesh mesh;
	...
	for (size_t x = 0; x < meshInfos.size(); x++)
	{
		...
		curAcc.type = gltf::Accessor::Type::Scalar;
		curAcc.componentType = gltf::Accessor::ComponentType::UnsignedShort;
		m_document.accessors.push_back(curAcc);

		gltf::Primitive curPrim;
		curPrim.indices = curAccIndex;
		curPrim.material = findOrCreateMaterial(meshInfos[x].matName);
		curPrim.mode = gltf::Primitive::Mode::Triangles;
...
ModelExporterGltf::addVertexDataRigged(const ModelBodyInternal& body, size_t vertCount)
{
	...
	bufView.target = gltf::BufferView::TargetType::ArrayBuffer;
	m_document.bufferViews.push_back(bufView);
	...
	uv1Acc.type = gltf::Accessor::Type::Vec2;
	uv1Acc.componentType = gltf::Accessor::ComponentType::Float;
	m_document.accessors.push_back(uv1Acc);
	curByteOffset += ModelBodyInternal::UVLen;

	gltf::Accessor uv2Acc;
	...
	uv2Acc.type = gltf::Accessor::Type::Vec2;
	uv2Acc.componentType = gltf::Accessor::ComponentType::Float;
	m_document.accessors.push_back(uv2Acc);
	curByteOffset += ModelBodyInternal::UVLen;

	gltf::Accessor boneIndexAcc;
	...
	boneIndexAcc.type = gltf::Accessor::Type::Vec4;
	boneIndexAcc.componentType = gltf::Accessor::ComponentType::UnsignedShort;
	m_document.accessors.push_back(boneIndexAcc);
	curByteOffset += ModelBodyInternal::boneIndexLen;

	gltf::Accessor boneWeightAcc;
	...
	boneWeightAcc.type = gltf::Accessor::Type::Vec4;
	boneWeightAcc.componentType = gltf::Accessor::ComponentType::Float;
...
ModelExporterGltf::addIndexData(const std::vector<uint16_t>& body)
{
	...
	bufView.byteOffset = startOffset;
	bufView.byteLength = byteLen;
	bufView.target = gltf::BufferView::TargetType::ElementArrayBuffer;
	m_document.bufferViews.push_back(bufView);
