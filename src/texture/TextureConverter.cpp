...
TextureConverter::convertHeader(TextureHeaderExternal& headerExternal, size_t fileSize)
{
...
TextureConverter::getCorrectResolution(uint16_t baseWidth, uint16_t baseHeight, bool isCompressed, uint32_t textureLength, int blockSize)
{
...
uint32_t
TextureConverter::getMip0Len(uint16_t width, uint16_t height, bool isCompressed, int blockSize)
{
	if (!isCompressed)
