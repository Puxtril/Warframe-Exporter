...
void
AnimationReader128::readHeaderDebug(BinaryReaderBuffered* headerReader, const Ensmallening& ensmalleningData, const LotusLib::CommonHeader& header)
{
...
void
AnimationReader128::readHeader(BinaryReaderBuffered* headerReader, const Ensmallening& ensmalleningData, const LotusLib::CommonHeader& header, AnimationHeaderExternal& outHeader)
{
	uint32_t pathLen1 = headerReader->readUInt32();
	headerReader->seek(pathLen1, std::ios_base::cur);
	uint32_t pathLen2 = headerReader->readUInt32();
...
void
AnimationReader128::isPureChannelType(uint16_t* channelTypes, int channelCount)
{
...
void
AnimationReader128::readBodyDebug(BinaryReaderBuffered* bodyReader, const AnimationHeaderExternal& extHeader, const Ensmallening& ensmalleningData, const LotusLib::CommonHeader& header)
{
	// Strange data
	...
	for (uint32_t x = 0; x < extHeader.skeletons.size(); x++)
	{
		...
		for (uint32_t y = 0; y < curSkel.actions.size(); y++)
		{
			...
			bodyReader->seek(curSkel.bones.size() * 16 * 4, std::ios_base::cur);
...
void
AnimationReader128::readBody(BinaryReaderBuffered* bodyReader, const AnimationHeaderExternal& extHeader, const Ensmallening& ensmalleningData, const LotusLib::CommonHeader& header, AnimationBodyExternal& outBody)
{
	...
	for (uint32_t x = 0; x < extHeader.skeletons.size(); x++)
	{
		...
		for (uint32_t y = 0; y < curSkel.actions.size(); y++)
		{
			...
			for (size_t x = 0; x < curSkel.bones.size(); x++)
			{
				...
				bodyReader->seek(0x14, std::ios_base::cur);
			}
			...
			for (size_t z = 0; z < curActionHead.frameCount; z++)
			{
				for (size_t w = 0; w < curSkel.bones.size(); w++)
				{
					...
					// This is it. This is what's preventing animations from being extracted.
					// Obviously, this is currently incorrect
					case 2:
					{
						uint16_t short1 = bodyReader->readUInt16();
						int16_t x = (short1 & 0x7FFE) >> 1;
						if (short1 & 0x4000)
							x |= 0xC000;
						
						uint16_t short2 = bodyReader->readUInt16();
						uint16_t y = (short2 & 0x3FFF);

						uint16_t short3 = bodyReader->readUInt16();
						uint16_t z = (short3 & 0xFFFC) >> 2;

						glm::quat rot( 1.0, x / 8192.0F, y / 16384.0F, z / 16384.0F);
