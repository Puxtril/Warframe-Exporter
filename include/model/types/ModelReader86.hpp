...
{
	class ModelReader86 : public ModelReader
	{
		ModelReader86() = default;

	public:
		inline static ModelReader86* getInstance()
		{
			static ModelReader86* instance = new ModelReader86();
			return instance;
		}

		inline std::vector<int> getEnumMapKeys() const override
		{
			std::vector<int> extTypes = { (int)ModelType::MODEL_STATIC_86 };
			return extTypes;
		}
	...
	//			throw unknown_format_error("Did not reach end of file");
		...
		void readBody(const ModelHeaderExternal& extHeader, BinaryReaderBuffered* bodyReader, ModelBodyExternal& outBody) override
		{
			...
			outBody.colors.resize(2);
			outBody.colors[0].resize(extHeader.vertexCount);
			outBody.colors[1].resize(extHeader.vertexCount);
			for (uint32_t x = 0; x < extHeader.vertexCount; x++)
			{
				...
				// Tangent or Vertex Colors?
				outBody.colors[1][x][0] = bodyReader->readUInt8();
				outBody.colors[1][x][1] = bodyReader->readUInt8();
				outBody.colors[1][x][2] = bodyReader->readUInt8();
