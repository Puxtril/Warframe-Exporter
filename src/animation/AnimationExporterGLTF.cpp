...
gltfAnimation::createBones(const std::vector<BoneTreeNodeExternal>& boneTree, const AnimationActionInternal& action)
{
	...
	for (size_t x = 0; x < boneTree.size(); x++)
	{
		...
		//std::memcpy(&boneNode.rotation[0], &action.getInitialTransform().rot[x][0], sizeof(float) * 4);
		//std::memcpy(&boneNode.translation[0], &action.getInitialTransform().pos[x][0], sizeof(float) * 3);
		//std::memcpy(&boneNode.scale[0], &action.getInitialTransform().scale[x][0], sizeof(float) * 3);
